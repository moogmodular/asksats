...
type ListTemplate = 'ending_soon' | 'newest' | 'public_settled' | 'default_template'
...
export const useListStore = create<List>((set) => ({
    ...
    setTemplate: (template: ListTemplate) => {
        const fun = {
            ending_soon: () => {
                set({ filterFor: 'OPEN', orderBy: 'deadline', orderByDirection: 'asc' })
            },
            newest: () => {
            ...
            default_template: () => {
                set({ orderBy: 'creation', orderByDirection: 'desc' })
            },
        }[template]
        fun()
    },
